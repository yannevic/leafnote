...
export default function HouseCalibrate() {
  ...
  const [wallC, setWallC] = useState(1.845)
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', marginBottom: 20 }}>
        ...
        <div style={{ background: '#16213e', padding: 16, borderRadius: 12 }}>
          <div style={{ color: '#e06c75', fontWeight: 700, marginBottom: 12 }}>🧱 Parede</div>
        ...
        <div style={{ background: '#16213e', padding: 16, borderRadius: 12 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20 }}>
            <Slider label="X" value={floorX} min={-600} max={600} onChange={setFloorX} />
            <Slider label="Y" value={floorY} min={-600} max={600} onChange={setFloorY} />
          </div>
        </div>
      ...
      >
        ...
        <div style={{ marginTop: 8 }}>// PISO (offset adicional)</div>
        <div>
        ...
        <div>
          // parede top também soma allY: <strong style={{ color: '#61afef' }}>{allY}</strong>
        </div>
      </div>

      {/* Cena */}
      <div style={{ background: '#2d1b4e', borderRadius: 16, padding: 20, overflow: 'auto' }}>
        <div style={{ position: 'relative', width: 900, height: sceneH }}>
          {wallTiles.map((pos, i) => (
            <div
              key={`w${i}`}
              style={{
                ...
                outline: '1px solid rgba(240,192,64,0.5)',
